...
const bootstrapData = () => {
    ...
    try {
        ...
        const allPermission = [];
        user.map( async(item) => {
            const result = await User.findOne({ email: item.email }).lean();
            if(!result){
                ...
                const org = await Organization.findOne({ name: item.organization });
                ...
                await User.updateOne({_id: result._id},userData );
            }
        });
    } catch (error) {
